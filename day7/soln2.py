...
def expression_eval(initial_number: int, operator: str, number: int) -> int:
    if operator == '||':
        return int(f'{initial_number}{number}')
    return eval(f'{initial_number}{operator}{number}')
...
def valid_expression(
    ...
    for number, operator in zip(numbers[1:], operators):
        if cum_result is None:
            cum_result = expression_eval(initial_number, operator, number)
        else:
            cum_result = expression_eval(cum_result, operator, number)
